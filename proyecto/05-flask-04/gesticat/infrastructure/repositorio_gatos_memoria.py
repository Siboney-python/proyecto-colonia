...
class RepositorioGatosMemoria(RepositorioGatos):
    ...
    def insertar(self, gato):
        """Inserta un gato nuevo. Lanza GatoYaExisteError si el id ya existe.
        ...
        if gato.id_gato in self._gatos:
            raise GatoYaExisteError(f"Ya existe un gato con id {gato.id_gato}.")
        self._gatos[gato.id_gato] = gato

    def actualizar(self, gato):
        """Actualiza un gato existente. Lanza GatoNoEncontradoError si el id no existe.
        ...
        if gato.id_gato not in self._gatos:
            raise GatoNoEncontradoError(f"No existe ningún gato con id {gato.id_gato}.")
        self._gatos[gato.id_gato] = gato
    ...
    def quitar(self, id_gato):
        """Elimina el gato con ese id. Lanza GatoNoEncontradoError si no existe."""
        if id_gato not in self._gatos:
            raise GatoNoEncontradoError(f"No existe ningún gato con id {id_gato}.")
